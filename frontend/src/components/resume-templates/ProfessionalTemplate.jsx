...
export default function ProfessionalTemplate({ resume }) {
  ...
  return (
    <div className="font-serif text-slate-900 bg-white p-8 sm:p-10 text-[13px] leading-relaxed mx-auto" style={{ maxWidth: '800px' }}>
      {/* Header */}
      <header className="text-center mb-6">
        <h1 className="text-3xl sm:text-4xl uppercase font-bold tracking-widest mb-2">{c.full_name}</h1>
          ...
          {resume.experience.map((e, i) => (
            <div key={i} className="mb-4">
              <div className="flex justify-between items-baseline mb-1">
                ...
                <span className="text-slate-600 whitespace-nowrap italic">{e.duration}</span>
              </div>
          ...
          {resume.education.map((e, i) => (
            <div key={i} className="mb-2">
              <div className="flex justify-between items-baseline">
                ...
                <span className="text-slate-600 whitespace-nowrap italic">{e.year}</span>
              </div>
